    ...
    <section class="content">
      ...
      <a-row class="mt-20">
        <a-col :span='4' :offset='1'>
          <a-button class="mr-10" @click='confirm' type="primary">通过</a-button>
          <a-button class="ml-10" @click='refuse' type="primary">不通过</a-button>
        </a-col>
      </a-row>
      <a-row class="mt-10">
  ...
  methods: {
    ...
    confirm(){
      ...
      self.$confirm({
        title: '确认通过此供应商吗？',
        onOk() {
          audit(self.supply_id).then(res=>{
            self.$message.success('审核通过，供应商已入库');
            let time = setTimeout(()=>{
              self.$router.go(-1);
              clearTimeout(time)
            },1500)
    ...
    refuse(){
      ...
      self.$confirm({
        title: '确认不通过此供应商吗？',
        onOk() {
          refuse(self.supply_id,self.remark).then(res=>{
