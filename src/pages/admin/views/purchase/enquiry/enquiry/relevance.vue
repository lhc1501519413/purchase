<template>
  <a-form id="relevance" :form="form" @submit="handleSubmit">
    <h5>
      <span>在线询价 / 询价单管理 / 关联询价单</span>
    ...
    <h3>
      <div>
        关联询价单编号：{{code}}
        <!-- <span class="ml-20">
          关联询价单编号：{{formData.relevance_code}}
        </span> -->
      </div>
      <div></div>
    </h3>
    ...
    <a-modal
      ...
      <a-tree-select
        ...
        dropdownMatchSelectWidth
      />
      <a-input placeholder="模糊查询：编号/名称/简称" style="width:20%;" class="ml mr pl" v-model="kw_code"></a-input>
...
export default {
  ...
  methods: {
    ...
    del(stock_id) {
      let index = this.selectedRowKeys.indexOf(stock_id);
