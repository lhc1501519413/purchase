  ...
  <div id="add_win">
    ...
    <section class="content">
      ...
      <div class="text-right bottom-date">
        <a-date-picker class="ml-10 mr-10" v-model="sign_date" :format="dateFormat"/>
      </div>
      ...
      <div class="mb-10">
        ...
        中标签领单附件：上传的中标签领单为签章后的文件，未签章文件视为无效文件。点击<a @click="signModalVisible = true">去签章</a>
      </div>
      <upload
        class="ml-10"
      ...
      <ul class="ml-10">
        <li
          class="mt-10"
          @click.stop="del"
          v-for="(item,index) of sign_file_list"
          :key="index"
        >
          <svg-icon class="wenjian" icon-class="wenjian" />
          <span class="ml-10 mr-10" style="width:10px;">{{item.file_name}}</span>
  ...
  methods: {
    get_win_notice_info(){
      get_win_notice_info({ id: this.id }).then(res=>{
        ...
        this.sign_date = res.data.sign_date?this.$moment(res.data.sign_date):null;
    ...
    save(){
      ...
      formData.sign_date = this.sign_date&&this.$moment(this.sign_date).format('YYYY-MM-DD');
      formData.sign_file_list = this.sign_file_list;
    ...
    del() {
      ...
      if (index2 !== undefined) {
        this.sign_file_list.splice(index2, 1);
      }
    }
  },
};
