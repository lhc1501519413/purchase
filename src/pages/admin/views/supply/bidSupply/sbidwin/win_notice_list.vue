      ...
      <a-table class="table" :dataSource="dataSource" :columns="columns" :pagination="false" rowKey="id">
        ...
        <template slot="operation" slot-scope="text,record">
          <div v-if="record.status==4">
            <router-link v-if="priv.swin_notice_list.edit" :to="{path:'/swin_confirm',query:{id:record.id}}">
              确认
            </router-link>
            <a v-if="priv.swin_notice_list.submit" @click="submit(record.id)">提交</a>
          </div>
          <div v-else>
            <router-link v-if="priv.swin_notice_list.view" :to="{path:'/swin_detail',query:{id:record.id}}">
              详情
            </router-link>
          </div>
        </template>
